(function(){
      ...
      function buildSearchIndex(rows){
        ...
        return items.map(function(entry){
          var raw = entry || {};
          var fundortId = String(raw.fundortId || raw.web_id || raw.id || raw.webId || '').trim();
          var address = String(raw.address || raw.publicText || raw.title || raw.location || '').trim();
          var species = Array.isArray(raw.species) ? raw.species : (raw.species ? String(raw.species).split(/\s*,\s*|\s*;\s*/) : []);
          var status = Array.isArray(raw.status) ? raw.status : (raw.status ? String(raw.status).split(/\s*,\s*|\s*;\s*/) : []);
          var building = String(raw.buildingType || raw.buildingName || '').trim();
          var description = String(raw.description || '').trim();
          var title = String(raw.title || address || raw.publicText || '').trim();
          var textParts = [
            fundortId,
            title,
            address,
            String(raw.publicText || ''),
            building,
            description,
            species.join(' '),
            status.join(' '),
          ...
          var searchText = normalizeSearchText(textParts.join(' '));
          return { fundortId: fundortId, title: title || address || 'Fundort', searchText: searchText };
        }).filter(function(item){ return item.fundortId || !!item.searchText; });
      }
      function getExactFundortIdMatches(index, query){
        var normalizedQuery = normalizeSearchText(query);
        if(!normalizedQuery){ return []; }
        return (index || []).filter(function(item){
          var id = String(item && item.fundortId || '').trim();
          return !!id && normalizeSearchText(id) === normalizedQuery;
        });
      }
      function filterMatches(index, query){
        var normalizedQuery = normalizeSearchText(query);
        if(!normalizedQuery){ return []; }
        var terms = normalizedQuery.split(/\s+/).filter(Boolean);
        var exactMatches = getExactFundortIdMatches(index, normalizedQuery);
        var matches = [];
        (index || []).forEach(function(item){
          if(!item){ return; }
          var idValue = String(item.fundortId || '').trim();
          var exactId = !!idValue && normalizeSearchText(idValue) === normalizedQuery;
          if(exactId){
            matches.push(item);
            return;
          }
          var text = item.searchText || '';
          if(terms.every(function(term){ return text.indexOf(term) !== -1; })){ matches.push(item); }
        });
        matches.sort(function(a, b){
          var aExact = !!(String(a && a.fundortId || '').trim()) && normalizeSearchText(a.fundortId) === normalizedQuery;
          var bExact = !!(String(b && b.fundortId || '').trim()) && normalizeSearchText(b.fundortId) === normalizedQuery;
        ...
        if(exactMatches.length && matches.length){
          ...
          matches.forEach(function(item){
            var key = String(item && item.fundortId || item && item.title || '');
